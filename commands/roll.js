...
module.exports = {
  execute: async (client, channel, tags, extraParams) => {
    try {
      ...
      if (!amount) {
        client.say(channel, `@${tags.username}, usage: !roll <amount>`)
        return
      }

      const parsedAmount = Number(amount)
      if (!Number.isInteger(parsedAmount) || parsedAmount <= 0) {
        client.say(channel, `@${tags.username}, roll amount must be a positive whole number.`)
        return
      }

      const text = await callApi('roll', channel, tags, [{ amount: parsedAmount }])
      client.say(channel, `@${tags.username}, ${text}\u200B`)
    } catch (err) {
      const code = err.message || 'UNKNOWN'
      const errorMessage = getErrorMessage(code, tags.username)
      console.error('❌ Error in roll:', code)
