...
module.exports = {
  execute: async (client, channel, tags, extraParams) => {
    ...
    if (!isAdminUser(username)) {
      client.say(
        channel,
        `@${tags.username}, freespintest is admin-only. Allowed admins: ${ADMIN_USERS.join(', ')}`,
      )
      return
    }
    ...
    try {
      ...
      client.say(channel, `@${tags.username}, [FREESPIN TEST] ${text}\u200B`)
    } catch (err) {
      const code = err.message || 'UNKNOWN'
      const errorMessage = getErrorMessage(code, tags.username)
      console.error('❌ Error in freespintest:', code)
